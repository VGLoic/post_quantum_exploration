...
mod merkletree_tests {
    ...
    impl BranchV0 {
        fn try_to_add(
            ...
            if let Err(_) = self.try_to_add_left(v, current_depth, depth) {
        ...
        fn try_to_add_left(
            ...
            match &mut self.left_node {
                ...
                Some(node) => match &mut **node {
                    NodeV0::Leaf(_) => {
                        return Err(anyhow!("can not add here"));
                    }
                    NodeV0::Branch(b) => {
                        return b.try_to_add(v, current_depth + 1, depth);
                    }
                },
            };
        }
    ...
    impl MerkleTreeV0 {
        fn add(&mut self, v: [u8; 32]) -> Result<(), anyhow::Error> {
            if let Err(_) = self.root.try_to_add_left(v, 0, self.depth) {
                if let Err(_) = self.root.try_to_add_right(v, 0, self.depth) {
    ...
    impl MerkleTreeV0 {
        ...
        fn get<'a>(&'a self, selector: &'a [bool]) -> Result<ValueWithProof<'a>, anyhow::Error> {
            ...
            let mut iterator = selector.into_iter();
            let first = iterator
                .next()
    ...
    fn test_v0() {
        ...
        for i in 0..number_of_elements {
            ...
            assert_eq!(selected.value, &values[i]);
