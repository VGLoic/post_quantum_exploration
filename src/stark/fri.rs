...
pub fn low_degree_proof<const N: u32>(
    ...
    for _ in 0..indirect_steps_count {
        ...
        for i in &row_excluded_indices {
            column_excluded_indices.insert(i % (units_in_row.len() / 4));
        }
        ...
        for unit_index in 0..(units_in_row.len() / 4) {
            ...
            for i in 0..4 {
                let index = (unit_index + i * units_in_row.len() / 4) % units_in_row.len();
        ...
        units_in_row = units_in_row.into_iter().skip(3).step_by(4).collect();
